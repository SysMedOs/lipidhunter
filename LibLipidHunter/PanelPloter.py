...
def plot_spectra(mz_se, xic_dct, ident_info_dct, ms1_rt, ms2_rt, ms1_df, ms2_df,
                 specific_check_dct, isotope_checker_dct, isotope_score, _pl_type,
    ...
    if len(_ms1_pr_df) > 0:
        ...
        if _isotope_df.shape[0] == 0 or _ms1_pr_df.get_value(_ms1_pr_df.index[0],
            ...
            ms_zoom_pic.stem(_ms_zoom_df['mz'].tolist(), _ms_zoom_df['i'].tolist(), 'grey', markerfmt=' ')
            ...
            if _ident_table_df.shape[0] > 0:
                _ident_table_df = _ident_table_df[['Proposed_structures', 'Score']].head(8)
